...
public class TenguAI : MonoBehaviour
{
    ...
    [Header("Físicas de Gravedad Pesada")]
    [SerializeField] private float escalaGravedadNativa = 3.5f;
    [SerializeField] private float fuerzaCaidaLibre = 1.8f;
    ...
    [SerializeField] private float maxHeightDifference = 1.8f; //
    ...
    void Awake()
    {
        ...
        if (rb != null)
        {
            rb.gravityScale = escalaGravedadNativa;
        }
    }
    ...
    void FixedUpdate()
    {
        ...
        if (rb != null)
        {
            ...
            if (velocidadY < 0.1f)
            {
                velocidadY += Physics2D.gravity.y * fuerzaCaidaLibre * Time.fixedDeltaTime;
            }

            rb.linearVelocity = new Vector2(moveDirectionX * speed, velocidadY); //
        }
    }
    ...
    public void DealDamage()
    {
        ...
        KitsuneController kitsune = player.GetComponent<KitsuneController>(); //
        if (kitsune != null && kitsune.IsInvisible) return; //
